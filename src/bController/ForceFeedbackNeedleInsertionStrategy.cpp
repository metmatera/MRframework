...
int ForceFeedbackNeedleInsertionStrategy::execTask() {
	...
	if (hIt != hapticListNames.end()) {
		...
		Eigen::Vector3f fb_m = f_feedback[0];
		Eigen::Vector3f friction = f_feedback[1];
		Eigen::Vector3f elastic = f_feedback[2];
		Eigen::VectorXf f_mat_flat(9);
		f_mat_flat << fb_m(0), fb_m(1), fb_m(2), 
					  friction(0), friction(1), friction(2), 
					  elastic(0), elastic(1), elastic(2);
		this->forcefeedback = f_mat_flat;
		...
		this->vrep->setFloatSignal("friction", friction(1), simx_opmode_oneshot, this->simPort);
		this->vrep->setFloatSignal("elastic", elastic(1), simx_opmode_oneshot, this->simPort);
	}
